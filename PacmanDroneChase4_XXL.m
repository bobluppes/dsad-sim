...
showGhostTarget = 1;        % flag whether to show where each ghost is heading towards or not
...
enemies(2).plot = imagesc(myAxes1,'XData',[enemies(2).pos(1)-0.6 enemies(2).pos(1)+0.6],'YData',[enemies(2).pos(2)+0.6 enemies(2).pos(2)-0.6],'CData',ghostSprites{1,2,1});
...
enemies(3).plot = imagesc(myAxes1,'XData',[enemies(3).pos(1)-0.6 enemies(3).pos(1)+0.6],'YData',[enemies(3).pos(2)+0.6 enemies(3).pos(2)-0.6],'CData',ghostSprites{1,2,1});
...
enemies(4).plot = imagesc(myAxes1,'XData',[enemies(4).pos(1)-0.6 enemies(4).pos(1)+0.6],'YData',[enemies(4).pos(2)+0.6 enemies(4).pos(2)-0.6],'CData',ghostSprites{1,2,1});
...
%ghostMode.tiles = [1 -10; 70 -10; 70 80; 1 80]; % corner tiles for scatter mode. are slightly above and beneath the actual corners so host don't get trapped 
...
tasktable = zeros(1, lives.orig);
distancetable = zeros(1, lives.orig);
...
%             end
    ...
    function pacmanMoveFun
        ...
        if autoPlay
            ...
            curSquare2 = pacmanAI4(pacman,enemies,allDirections,coins,pills);
        ...
        for nn = 1:4 % consider one ghost at a time
            ...
            switch enemies(nn).status % handle ghost status 1 to 7
                ...
                case 2 % grumpy mode
                    ...
                    enemies(nn) = pathWayLogic(enemies(nn),overallEnemySpeed*1);
                case 3 % eye mode
                    if enemies(nn).dir > 0
                        enemies(nn).oldDir = enemies(nn).dir;
                    end

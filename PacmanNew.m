...
newEnemyTime = 100;         % time-increments that pass before the next ghost is let out of his cage (default: 500)
fruitAppear = [0,0];   % time frame in whih fruits are to appear in the game (default: between 300 and 1500 time-increments after level start)
...
pacman.speed = 1/2;         % pacman speed (default: 1/6 => pacman eats exactly two coins with every mouth open/close cycle, maximum possible: 1/2)
...
score.data = 0;             % score
score.plot = text(29,33.1,['Detected: ' num2str(pacman.detected)],'Color','w','FontSize',12,'HorizontalAlign','Right','FontName',pacFont,'FontUnits','normalized','FontWeight','bold','Parent',myAxes1);
...
pacmanLabyCreator_Fig = figure('Visible','off');
    ...
    function newGameButtonFun
        ...
        score.data = 0;
        set(score.plot,'String',['Detected: ' num2str(pacman.detected)])
    ...
    function GameLoop
        ...
        detectedChart(ii) = pacman.detected;
        ii = ii + 1;
        if (pacman.detected == 4)
           figure;
           plot(detectedChart);
           title('Intruders detected in Port of Rotterdam');
           xlabel('steps');
           ylabel('number of intruders detected');
           error('Finished');
        end
    end
...
%                 pacman.status = -3; % pacman dies
                  pacman.detected = pacman.detected + 1;
                  enemies(nn).status = 8;
                  set(enemies(nn).plot,'Visible','off')
            end
